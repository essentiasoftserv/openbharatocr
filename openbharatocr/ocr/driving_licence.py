...
def extract_all_names(input):
    ...
    stopwords = ['INDIA', 'OF', 'UNION', 'PRADESH', 'TRANSPORT', 'DRIVING', 'LICENCE', 'FORM', 'MCWG', 'LMV', 'TRANS', 'ANDHRA', 'UTTAR', 'MAHARASHTRA', 'GUJARAT', 'TAMIL', 'NADU',
                 'WEST', 'BENGAL', 'KERELA', 'KARNATAKA', 'DRIVE', 'AUTHORIZATION', 'FOLLOWING', 'CLASS', 'DOI']

    names = [name.strip() for name in cleaned if not any(word in name for word in stopwords) and len(name.strip()) > 3]
...
def extract_auth_allowed(input):
    ...
    auth_types, auth_allowed = ['MCWG', 'M.CYL.', 'LMV-NT', 'LMV', 'TRANS', 'INVCRG'], []
